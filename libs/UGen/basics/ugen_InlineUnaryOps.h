// $Id: ugen_InlineUnaryOps.h 350 2011-07-22 09:07:56Z 0x4d52 $
// $HeadURL: http://ugen.googlecode.com/svn/trunk/UGen/basics/ugen_InlineUnaryOps.h $
...
#elif defined(UGEN_ANDROID)
inline float log2 (const float a) throw()			{	return (float)(logf(a) * (float)oneOverLog2);		}
inline double log2 (const double a) throw()			{	return log(a) * oneOverLog2;						}
#else
...
inline float ampdb(const float amp) throw()			{	return (float)::log10(amp) * 20.f;							}
...
inline double deg2rad(const double deg) throw()		{	return deg * oneOver360 * twoPi;							}
inline float rad2deg(const float deg) throw()		{	return deg * (float)oneOverTwoPi * 360.0;					}
inline double rad2deg(const double deg) throw()		{	return deg * oneOverTwoPi * 360.f;							}
